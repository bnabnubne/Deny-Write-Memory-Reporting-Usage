...
static int sys_create (const char *file_name, unsigned initial_size);
static int sys_remove (const char *file_name);
...
{
  tid_t tid;
  char *kfile = copy_in_string (cmdline);
      ...
      if (fd->file != NULL)
        {
          ...
          list_push_front (&cur->list_fds, &fd->elem);
        }
      else
        free (fd);
      lock_release (&fs_lock);
...
lookup_fd (int handle)
{
  ...
  for (e = list_begin (&cur->list_fds); e != list_end (&cur->list_fds);
       e = list_next (e))
    {
      struct file_descriptor *fd;
...
syscall_exit (void)
{
  ...
  for (e = list_begin (&cur->list_fds); e != list_end (&cur->list_fds); e = next)
    {
